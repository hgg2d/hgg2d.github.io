...
class Chan {
  ...
  constructor() {
    ...
    this.lewdsToggle = this.hgg2d.querySelector('.hgg2d__lewdsToggle > a');
    this.settingsToggle = this.hgg2d.querySelector('.hgg2d__settingsToggle > a');
    this.settingsBox = this.hgg2d.querySelector('.hgg2d__settings');
    this.prev = this.hgg2d.querySelector('.hgg2d__follow');
    [this.input_previewBar, this.input_previewGrid] = Array.from(this.hgg2d.querySelectorAll('input'));
  ...
  createMatches(phrase, url) {
    const matchForm = this.hgg2d.querySelector('.hgg2d__matchForm');
  ...
  html() {
    ...
    this.hgg2d.innerHTML = (`
      ...
      <aside class="hgg2d__codes ${this.settings.previewBar ? '' : 'hgg2d-hidden'}">
  ...
  initEventListeners() {
    ...
    this.hgg2d.querySelector('.hgg2d__settings-close').addEventListener('click', () => {
      this.settingsBox.classList.toggle('hgg2d-hidden');
    });

    this.input_previewBar.addEventListener('click', (e) => {
    ...
    this.input_previewGrid.addEventListener('click', (e) => {
      ...
      this.lewdsToggle.textContent = this.settings.previewGrid ? 'hide' : 'show';
    });

    this.lewdsToggle.addEventListener('click', () => {
      this.settings.previewGrid = this.lewdsToggle.textContent === 'show';
      this.lewdsToggle.textContent = this.settings.previewGrid ? 'hide' : 'show';
      if (this.settings.previewGrid) {
        this.lewds.classList.remove('hgg2d__nogrid');
      } else {
    ...
    this.settingsBox.addEventListener('click', (e) => {
      if (e.target.nodeName !== 'BUTTON') {
        return;
      }
      /** @type {HTMLDivElement} */
      const parent = e.target.parentNode;
      const action = e.target.textContent === 'add' ? 'add' : 'remove';
  ...
  over = (e) => {
    ...
    if (target.href.includes('dlsite')) {
      const code = target.href.match(/[RV]J\d{6}/)[0];
