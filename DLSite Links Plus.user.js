...
(() => {
    ...
    let RJCode = /((?:(?:dlsite|www|http|maniax)[^>\s]+)?r[je]((\d{3})\d{3})(?:\.html)?)/gi
    let RGBlog = /(?:http)\S*?b\S*rg(\d{5})([A-Za-z0-9-._~:/?#@!$&'()*+,;=`]*\.html)?/gi
    let RGCirc = /(?:(?:http|www)?\S*com\S*|^|\s)rg(\d{5})(?:\.html)?/gi
    ...
    let createCirc = (el, match, code) => {
        let anch = createAnch(match)
        anch.href = 'http://www.dlsite.com/maniax/circle/profile/=/maker_id/RG' + code + '.html'
        return anch
    }
    ...
    let createRJ = (el, match, text, code) => {
        ...
        anch.href = (text.includes('announce'))
            ? ('http://www.dlsite.com/maniax/announce/=/product_id/RJ' + code + '.html')
            : ('http://www.dlsite.com/maniax/work/=/product_id/RJ' + code + '.html')
        anch.classList.add('lewds')
        return anch
    }
    ...
    let hover = (e) => {
        ...
        if (e.target.href.includes('dlsite')) {
            let code = e.target.href.split('/RJ')[1].split('.')[0]
            ...
            let code = e.target.href.split('cid=')[1].split('/')[0]
