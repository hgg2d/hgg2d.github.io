...
class Chan {
  ...
  DMMCode = /(?:(?:dmm|www|https?)[^>\s]+)?(?:cid=)?(?:d_|DMM)(\d{6,8})\/?/gi;
  RGCirc = /(?:(?:http|www)?\S*com\S*)?[rv]g(\d{5})(?:\.html)?/gi;
  RJCode = /(?:(?:http|www|dlsite)[^>\s]+)?[vr][je]((\d{3})\d{3,5})(?:\.html)?/gi;
  ...
  addPreview(src, link) {
    ...
    img.addEventListener('error', () => {
      ...
      if (src.includes('dlsite')) {
        let code = src.match(/[RV][JE]\d{6,8}/g)[1];
  ...
  createRJ(match, code, bucket) {
    ...
    this.games.add(bar);
    this.addCode(anchor, bar);
    var round = Number(String(code).slice(0, String(code).length - 3));
    if (!(Number(code) % 1000 == 0)) round += 1;
    const padded = this.padLeft(round, code.toString().length - 3);
    const pathType = type === 'work' ? 'work' : 'ana';
    const circlePathType = circleType === 'pro' ? 'professional' : 'doujin';
    const src = `https://img.dlsite.jp/modpub/images2/${pathType}/${circlePathType}/${prefix}${padded}000/${prefix}${code}${pathType === 'ana' ? '_ana' : ''}_img_main.jpg`;
    this.addPreview(src, anchor.href);
    return anchor;
  }
  ...
  over = (e) => {
    ...
    if (target.href.includes('dlsite')) {
      const code = target.href.match(/[RV][JE]\d{6,8}/)[0];
